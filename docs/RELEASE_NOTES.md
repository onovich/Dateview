...
- Limited preview testers should use `docs\PREVIEW_RELEASE_HANDOFF.md`; preview feedback should use `docs\PREVIEW_FEEDBACK_GUIDE.md`.
...
- Verify a bundle by comparing `Get-FileHash -Algorithm SHA256 .\Dateview-0.1.0-preview-win-x64.zip` with the matching `.sha256.txt` file. The exact current preview artifact hash is recorded in `docs\P11_LIMITED_PREVIEW_RELEASE_VALIDATION.md`.<br/>**校验发布包时，请将 `Get-FileHash -Algorithm SHA256 .\Dateview-0.1.0-preview-win-x64.zip` 的结果与配套 `.sha256.txt` 文件对比。精确的当前预览产物哈希记录在 `docs\P11_LIMITED_PREVIEW_RELEASE_VALIDATION.md`。**
